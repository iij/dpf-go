...
// DsRecordsAPIService DsRecordsAPI service
type DsRecordsAPIService service

type ApiGetDsRecordListRequest struct {
	ctx        context.Context
	ApiService *DsRecordsAPIService
	zoneId     string
}

func (r ApiGetDsRecordListRequest) Execute() (*GetDsRecords, *http.Response, error) {
	return r.ApiService.GetDsRecordListExecute(r)
}

/*
GetDsRecordList DSレコードの一覧取得

上位のネームサーバに登録が必要なDSレコードを取得できます。

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param zoneId ID
	@return ApiGetDsRecordListRequest
*/
func (a *DsRecordsAPIService) GetDsRecordList(ctx context.Context, zoneId string) ApiGetDsRecordListRequest {
	return ApiGetDsRecordListRequest{
		ApiService: a,
		ctx:        ctx,
		zoneId:     zoneId,
...
//
//	@return GetDsRecords
func (a *DsRecordsAPIService) GetDsRecordListExecute(r ApiGetDsRecordListRequest) (*GetDsRecords, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *GetDsRecords
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "DsRecordsAPIService.GetDsRecordList")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/zones/{ZoneId}/ds_records"
