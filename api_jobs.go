...
func (r ApiGetJobRequest) Execute() (*GetJobs, *http.Response, error) {
	return r.ApiService.GetJobExecute(r)
}

/*
GetJob 非同期リクエストの状態確認
...
*/
func (a *JobsAPIService) GetJob(ctx context.Context, requestId string) ApiGetJobRequest {
	return ApiGetJobRequest{
		ApiService: a,
		ctx:        ctx,
		requestId:  requestId,
...
//
//	@return GetJobs
func (a *JobsAPIService) GetJobExecute(r ApiGetJobRequest) (*GetJobs, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *GetJobs
	)
	...
	localVarPath := localBasePath + "/jobs/{RequestId}"
	...
	if strlen(r.requestId) < 32 {
		return localVarReturnValue, nil, reportError("requestId must have at least 32 elements")
	}
	if strlen(r.requestId) > 32 {
