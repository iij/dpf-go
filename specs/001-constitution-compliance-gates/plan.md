...
`go-licenses` v2.0.1、`betterleaks` v1.8.1 を版固定で用いる。版の取得手段はツールごとに
異なり、`go-licenses` と `betterleaks` はビルド情報から読む（research.md D9・D11）
...
- **`make` の終了コードで「違反あり」と「検査できなかった」を区別できない。**
  GNU make はレシピ失敗時に必ず 2 を返す。区別は `make check` の実行順序（版検証を
  先頭にまとめる）と `RESULT` 行、およびツール単体の終了コードで表す（research.md D10）
- 版を取得するコマンドはシェル変数へ代入せず、レシピのコマンド位置へ直接展開する。
  代入時の二重展開で版が常に空になる（research.md D11）
- シークレット検査は `SECRET_SCAN_TARGET` 変数から呼ぶ。憲章が定めるツール名の変更に
  一括実行側を 1 行で追随させるため（gitleaks から betterleaks への移行で機能した）

**規模 / 範囲**: 5 モジュール、Go ファイル約 300（うち生成物が大半）、検査 4 種、
一括実行が呼ぶゲート 7 種、版を固定するツール 4 種、依存パッケージ 65 件
（重複除去後、import されるもの）、要件 30 件（FR）／成功基準 11 件（SC）。
`make check` の実測所要時間は約 25 秒
...
憲章 **v2.0.1** に対する評価。Phase 1 設計後、および**実装完了後**の再評価を含む。
v2.0.1 は削除された CLAUDE.md への参照を差し替えた PATCH で、規範は変わっていない。
v2.0.0 は脆弱性検査を
...
**違反なし。** Phase 1 設計後および実装完了後に再評価し、判定は変わっていない。

**実装完了後の確認**: `make check` が 7 ゲートすべてを通り（`RESULT: ok`、約 25 秒）、
検査前後で作業ツリーの差分は 0 件だった。憲章の品質ゲート表の 7 ターゲットはすべて
Makefile に存在する。`betterleaks` への移行が完了したことで、憲章と実装の字面の
不一致も解消した。

**実装中に判明した設計の是正**: 終了コードの契約（D10）と版検証の probe の展開（D11）を
Phase 0 に追記し、[contracts/gates.md](./contracts/gates.md) の共通の契約を書き直した。
当初の契約は各 `make` ターゲットの終了コードで 0 / 1 / 2 を区別すると定めていたが、
GNU make では実現できなかった。区別の意味は、実行順序・`RESULT` 行・ツール単体の
終了コードへ移した。

**新しいゲートが初回実行で検出したもの**: `misc/vault` の `go-jose/v4`（GO-2026-4945）と
`misc/gcp` の `google.golang.org/grpc`（GO-2026-6061）に到達可能な既知脆弱性があった。
憲章の定める対処（依存の更新）を行い、いずれも解消した。ゲートを置く前は
気づけていなかった。
...
したがって本計画は憲章の改訂を待つ必要がなく、**上表の 20 行はいずれも v2.0.1 の
